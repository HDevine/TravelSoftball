...
const Team18u = ({ children }) => {
  ...
  const fetchTeam = () => {
    ...
    fetch(`https://www.ehtsoftball.com/db/GetTravelTeam18U.php`)
    .then(async resp => {
...
//      fetch(`http://softball-pi4/db/GetTravelNews18U.php`)
  ...
  return (
    ...
    >
      <ModalHeader title={<Label color="green">Sarni 18U Events</Label>} description={eventTitle} descriptorId="events-description" labelId="events-description" />
      <ModalBody>
      ...
      <Tabs defaultActiveKey={0} aria-label="Team18U-Tabs" role="tab">
        ...
        {!teamLoading && teamData?.map(row => (
          ...
          <Card>
            {row.teamPic && <Brand src={row.teamPic} alt={row.name} heights="lg" widths="lg" />}
          </Card>
          </Gallery>
          </React.Fragment>
        ))}
